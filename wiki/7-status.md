...
| Non-incremental re-render drops the tail | Planned — defect | `text.startsWith(emitted)` guard skips the update and never recovers | [§3.3](./3-api/3.3-streaming.md) |
...
| Session pool, ≤2 pages per provider | Implemented | `maxPerProvider = 2`, FIFO waiter queue, one relaunch retry | [§5.4](./5-browser-providers/5.4-the-session-pool.md) |
| `maxPerProvider` configurable | Planned | Constructor default with no env var or flag | [§5.4](./5-browser-providers/5.4-the-session-pool.md) |
...
| Waiter queue timeout / cancellation | Planned | Waiters wait indefinitely; a disconnected client still holds its slot | [§5.4](./5-browser-providers/5.4-the-session-pool.md) |
